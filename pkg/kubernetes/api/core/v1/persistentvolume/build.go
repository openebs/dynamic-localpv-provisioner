...
// WithNodeAffinityHostname sets the NodeAffinity field of PV with provided node name
func (b *Builder) WithNodeAffinityHostname(nodeName string) *Builder {
	if len(nodeName) == 0 {
...
// WithNFS sets the NFS volume source settings
func (b *Builder) WithNFS(server, path string, readOnly bool) *Builder {
	if len(server) == 0 {
		b.errs = append(b.errs, errors.New("failed to build PV object: missing NFS Server address"))
		return b
	}
	if len(path) == 0 {
		b.errs = append(b.errs, errors.New("failed to build PV object: missing NFS Path"))
		return b
	}
	volumeSource := corev1.PersistentVolumeSource{
